...
def main(run_conf, da_conf, exp_dir):
    ...
    mnist_train_ds = get_mnist(
        "data", train=True, transform=Compose([ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    )
    mnistm_train_ds = get_mnistm(
        "data", train=True, transform=Compose([ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    )
    mnist_test_ds = get_mnist(
        "data", train=False, transform=Compose([ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    )
    mnistm_test_ds = get_mnistm(
        "data", train=False, transform=Compose([ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    )
    ...
    if da_config["da_type"] == "dann" and not da_config["da_spec_config"]["dann"]["auto_critic_update"]:
        ...
        clf_optim = torch.optim.Adam(
            list(clf_model.parameters()) + da.get_da_params(), lr=run_conf["lr"], weight_decay=run_conf["weight_decay"]
        )
    else:
    ...
    if Path(model_file).exists():
        ...
        for epoch in range(1, run_conf["epochs"] + 1):
            clf_loss, da_loss = train_step(
                src_train_dl, tgt_train_dl, da, clf_model, clf_optim, clf_criterion, run_conf["device"]
            )
            logger.info(
                f"EPOCH {epoch:03d}: clf_loss={clf_loss:.4f}, da_loss={da_loss:.4f}, "
...
def train_step(src_train_dl, tgt_train_dl, da, model, optimizer, criterion, device):
    ...
    for (src_x, src_y), (tgt_x, _) in tqdm(zip(src_train_dl, tgt_train_dl), total=len(src_train_dl)):
        ...
        dl = torch.cat((torch.zeros(len(src_x), dtype=torch.int64), torch.ones(len(tgt_x), dtype=torch.int64))).to(
            device
        )
...
if __name__ == "__main__":
    ...
    da_config = {
        ...
        "adv_config": {
            "da_net_config": {"layers_width": [100], "act_function": "relu", "dropout": 0.0},
            "da_optimizer_config": {"lr": 0.0001, "weight_decay": 0.001},
        },
        ...
        "da_spec_config": {
            ...
            "jdot": {"jdot_alpha": 1.0},
            "mmd": {"kernel_mul": 2, "kernel_num": 4, "fix_sigma": None},
            "swd": {"multiplier": 8, "p": 2},
            "wdgrl": {
                "gp_da_lambda": 1.0,
                "critic_iter": 5,
            },
